...
<style>
    ...
    /* Teks judul, label widget, dan keterangan yang menempel langsung di latar biru tua */
    ...
    /* Kotak input (angka, teks, dropdown) berlatar putih -> teks harus gelap */
    ...
    /* Header expander (mis. "Sumber Data") berlatar putih -> teks & ikon harus gelap */
    ...
    /* Semua teks/label DI DALAM badan expander (latar putih) -> harus gelap, bukan putih */
    section[data-testid="stSidebar"] [data-testid="stExpander"] * {{
        color: {WARNA_BIRU_TUA} !important;
    }}
    /* Kecuali teks tombol di dalam expander (mis. Unduh Template Excel) -> tetap putih di atas latar biru */
    section[data-testid="stSidebar"] [data-testid="stExpander"] .stDownloadButton button,
    section[data-testid="stSidebar"] [data-testid="stExpander"] .stDownloadButton button * {{
        color: {WARNA_PUTIH} !important;
    }}
    /* Kotak unggah file (drag & drop, teks instruksi, tombol Browse files) berlatar putih -> teks gelap */
    ...
    /* Tombol (termasuk tombol unduh) -> latar biru cerah, teks putih, tegas */
...
def info_checksum_csv():
    """Menghitung checksum & ukuran file CSV lokal untuk verifikasi versi data yang benar-benar terpakai."""
    if not os.path.exists("data_pabrik_simulasi.csv"):
        return None
    with open("data_pabrik_simulasi.csv", "rb") as f:
        isi = f.read()
    return {
        "checksum": hashlib.md5(isi).hexdigest()[:10],
        "ukuran_kb": round(len(isi) / 1024, 1),
        "waktu_modifikasi": pd.Timestamp(os.path.getmtime("data_pabrik_simulasi.csv"), unit="s"),
    }
...
def resample_tampilan(df, interval_label):
    ...
    Data mentah 15 menit tetap dipakai untuk KPI, Smart Recommendation,
    dan Simulasi Skenario agar akurasi deteksi beban puncak tidak hilang.
    """
    if interval_label == "15 Menit (Asli)":
        return df
    ...
    df_ringkas = df.set_index("Timestamp").resample(aturan).agg(aturan_agregasi).reset_index()
    return df_ringkas
...
info_csv = info_checksum_csv()
if info_csv:
    st.sidebar.caption(
        f"🧾 Versi data lokal: `{info_csv['checksum']}` · {info_csv['ukuran_kb']} KB · "
        f"diubah {info_csv['waktu_modifikasi']:%Y-%m-%d %H:%M}"
    )
...
interval_tampilan = st.sidebar.selectbox(
    ...
    index=1,
    help="Hanya meringkas tampilan tabel/grafik. KPI dan Smart Recommendation tetap memakai data 15 menit asli agar akurasi deteksi beban puncak terjaga.",
)
...
# df_tampil: versi diringkas KHUSUS untuk tabel & grafik (lihat interval_tampilan di sidebar)
# df (15 menit asli) tetap dipakai untuk KPI, Smart Recommendation, dan Simulasi Skenario
...
if modul == "Overview":
    ...
    if sumber_data == "Edit Manual di Tabel":
        ...
        st.caption(f"Menampilkan data dengan resolusi: **{interval_tampilan}**" + (
            " — SoC = rata-rata & Charging = *pernah terjadi* dalam periode ini (lihat catatan di modul Fleet AGV/EV)."
...
# ============================================================
    ...
    fig_line.add_trace(go.Scatter(x=df_tampil["Timestamp"], y=df_tampil["Solar_Power_kW"], name="Solar Power (kW)", fill="tozeroy", line=dict(color=WARNA_BIRU_CERAH), fillcolor="rgba(187,212,255,0.5)"))
...
# ============================================================
    ...
    if interval_tampilan != "15 Menit (Asli)":
        st.markdown(
            ...
            "ditampilkan adalah <b>rata-rata</b> selama periode tersebut, sedangkan status Charging "
            "menunjukkan apakah pengisian terjadi <b>kapan pun</b> dalam periode itu. Akibatnya, SoC pada "
            "satu periode bisa tampak lebih tinggi dari periode sebelumnya meski status Charging periode itu "
            "sudah <i>False</i> — ini wajar (pengisian baru saja selesai di awal periode). Untuk melihat "
            "korelasi SoC dan status Charging secara presisi per kejadian, pilih interval <b>15 Menit (Asli)</b> "
            "di sidebar.</div>",
            unsafe_allow_html=True,
        )
    fig_agv = go.Figure()
