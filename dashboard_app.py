...
# ============================================================
    ...
    fig_line = go.Figure()
    fig_line.add_trace(go.Scatter(x=df["Timestamp"], y=df["Daya_Mesin_kW"], name="Beban Mesin (kW)", line=dict(color="#DC2626")))
    fig_line.add_trace(go.Scatter(x=df["Timestamp"], y=df["Solar_Power_kW"], name="Solar Power (kW)", fill="tozeroy", line=dict(color="#16A34A")))
    ...
    with col_b:
        ...
        fig_bar = px.bar(emisi_harian, x="Timestamp", y="Emisi_CO2_kg", color_discrete_sequence=["#2563EB"])
...
# ============================================================
    ...
    fig_agv = go.Figure()
    warna = {"AGV_01": "#2563EB", "AGV_02": "#F59E0B", "AGV_03": "#9333EA"}
    for a in DAFTAR_AGV:
        fig_agv.add_trace(go.Scatter(x=df["Timestamp"], y=df[f"{a}_SoC"], name=a, line=dict(color=warna[a])))
    fig_agv.add_hline(y=min_soc, line_dash="dot", line_color="#DC2626", annotation_text="Batas Minimal")
    fig_agv.add_hline(y=max_soc, line_dash="dot", line_color="#16A34A", annotation_text="Target Penuh")
...
# ============================================================
    ...
    fig_sim = go.Figure()
    fig_sim.add_trace(go.Bar(name="Emisi Saat Ini", x=["Total Emisi CO2 (kg)"], y=[total_emisi], marker_color="#DC2626"))
    fig_sim.add_trace(go.Bar(name="Emisi Proyeksi", x=["Total Emisi CO2 (kg)"], y=[emisi_proyeksi], marker_color="#16A34A"))
